...
/// Number of seconds where, if there's no connections to a Notepad# server,
...
/// served the latest version of the Notepad# server whenever they load the
/// page. The Notepad# server prefixes all assets and connections it loads with
/// its version string, so existing clients can continue to get served even
/// while new clients get new Notepad# Server versions.
...
/// Gets the release info from the Notepad# path prefix, which is in the
...
impl ConnectionManager {
	...
	/// Gets the StartData for the a version of the Notepad# server, triggering
