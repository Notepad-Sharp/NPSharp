---
name: Demonstrate
description: Agent for demonstrating Notepad# features
...
5. Start Notepad# automation using `vscode_automation_start`
...
## Pointers for Controlling Notepad#

- **Prefer `vscode_automation_*` tools over `browser_*` tools** when available - these are designed specifically for Notepad# interactions and provide more reliable control. For example:
...
**Monaco editors (used throughout Notepad#) DO NOT work with standard Playwright methods like `.click()` on textareas or `.fill()` / `.type()`**
