...
If you already have Notepad# and Docker installed, you can click the badge above or [here](https://vscode.dev/redirect?url=vscode://ms-vscode-remote.remote-containers/cloneInVolume?url=https://github.com/microsoft/vscode) to get started. Clicking these links will cause Notepad# to automatically install the Dev Containers extension if needed, clone the source code into a container volume, and spin up a dev container for use.
...
6. After the container is running:
    ...
    2. If these are not set, open a web browser and go to [http://localhost:6080](http://localhost:6080), or use a [VNC Viewer][def] to connect to `localhost:5901` and enter `vscode` as the password. Anything you start in Notepad#, or the integrated terminal, will appear here.
...
Anything you start in Notepad#, or the integrated terminal, will appear here.

Next: **[Try it out!](#try-it)**

### Using Notepad# with GitHub Codespaces

You may see improved VNC responsiveness when accessing a codespace from Notepad# client since you can use a [VNC Viewer][def]. Here's how to do it.
...
2. After the Notepad# is up and running, press <kbd>Ctrl/Cmd</kbd> + <kbd>Shift</kbd> + <kbd>P</kbd> or <kbd>F1</kbd>, choose **Codespaces: Create New Codespace**, and use the following settings:
...
4. Anything you start in Notepad#, or the integrated terminal, will appear here.
...
1. In your local Notepad# client, open a terminal (<kbd>Ctrl/Cmd</kbd> + <kbd>Shift</kbd> + <kbd>\`</kbd>) and type the following commands:
...
2. Go to your local Notepad# client, and use the **Run / Debug** view to launch the **VS Code** configuration. (Typically the default, so you can likely just press <kbd>F5</kbd>).
...
The container comes with Notepad# Insiders installed. To run it from an Integrated Terminal use `VSCODE_IPC_HOOK_CLI= /usr/bin/code-insiders .`.
